...
\institute{
    ...
    \href{}{\tt \scriptsize 8 Décembre 2015}
}
\date{
...
\begin{frame}{user-space et kernel-space (2)}
    ...
    \vspace{10pt}
    Dans un processeur x86, la notion d'anneau est représentée via le champs
    ...
    \onslide<2->{
        \vspace{8pt}
        => le champs IOPL ne peut être modifié qu'avec le niveau de privilège le
        plus élevé!
    }
...
\begin{frame}{user-space et kernel-space (3)}
    ...
    \onslide<2->{
        \item Espace kernel : réservé pour le kernel et ses modules. Possède le
