...
\begin{frame}{Pipe (4)}
    ...
    \onslide<2->{
        ...
        une représentation au sein du système de fichier.
...
\begin{frame}{Mémoire partagée (3)}
    ...
    TODO : shm source code + dans filesystem
...
\begin{frame}{Sémaphore (4)}
    ...
    \vspace{10pt}
    De même que pour les tubes, un nom peut être associé à la sémaphore
    pour permettre à d'autres processus de l'utiliser!

    \vspace{10pt}
    \onslide<2->{
        Les sept appels systèmes :
        \bi
        \itemsep5pt
        \item {\textbf{sem\_init}} : initialisation de la sémaphore
        \item {\textbf{sem\_open}} x 2 : deux prototypes d'ouverture
                                        (créateur/utilisateur)
        \item {\textbf{sem\_close}} : fermeture de la sémaphore
        \item {\textbf{sem\_unlink}} : destruction de la sémaphore
        \item {\textbf{sem\_wait}} : attente bloquante de la sémaphore
        \item {\textbf{sem\_post}} : relache la sémaphore
        \ei
    }
...
\begin{frame}{Programmation réseau (3)}
    ...
    \onslide<2->{
        \vspace{8pt}
        Adress Resolution Protocol : trouver l'adresse Ethernet d'une machine
...
\begin{frame}{Programmation réseau (7)}
    ...
    \onslide<2->{
        ...
        \item Le port : integer
        \ei
    }

    \onslide<3->{
        \vspace{15pt}
        Une socket est manipulée via un file descriptor!
    }
...
\begin{frame}{Multiplexage d'entrées (1)}
    ...
    \vspace{15pt}
    Dans un système communiquant, un même process doit généralement gérer
    l'arrivée et l'envoie d'informations à travers plusieurs canaux de type
    différents.
    ...
    \onslide<2->{
        ...
        \vspace{15pt}
        => l'appel system {\textbf{select}} est utilisé pour le multiplexage!
    }
...
\begin{frame}{Multiplexage d'entrées (2)}
    ...
    \begin{enumerate}
    \itemsep8pt
    \item j'indique au kernel les fd que je souhaite écouter
