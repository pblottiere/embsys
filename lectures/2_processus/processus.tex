...
\begin{frame}{Identification (2)}
    ...
    TODO : exemple avec shell comme père
\end{frame}
...
\begin{frame}{Création de processus (3)}
    ...
    \item {\textbf{system}} : exécute une commande shell (à bannir sûr...)
...
\begin{frame}{Terminaison d'un processus (6)}
    ...
    \item {\textbf{signal}} : très simple, défini par Ansi C et SUSv4 mais peut
          avoir des problèmes de portabilité entre système UNIX
...
\begin{frame}{Les pthreads (9)}
    ...
    \onslide<2-> {
        EXAMPLE SORTIE gcc -pthread

        \vspace{10pt}
        L'option -D\_REENTRANT indique au compilateur que les fonctions peuvent
        être utilisée simultanément (évite la duplication en RAM).
    }
