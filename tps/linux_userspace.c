...
void led_green(int value) {
	...
	file1 = fopen("/sys/class/gpio/gpio27/value", "w");  //LED red
	file2 = fopen("/sys/class/gpio/gpio22/value", "w");  //LED green
	...
	if (file1 != NULL && file2 != NULL) {
		fputc(data[!value], file1);
		fputc(data[ value], file2);

		fclose(file1);
		fclose(file2);
	} else {
		printf("Impossible to open the GPIO");
	}
}
